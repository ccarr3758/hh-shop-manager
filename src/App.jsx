...
function OutlookCalendar({ jobs, ctx, reload, selectedDate, setSelectedDate }) {
  ...
  useEffect(() => {
    const tokenFromUrl = readOutlookTokenFromUrl();
    if (!tokenFromUrl) return;
    setGraphToken(tokenFromUrl);
    saveOutlookToken(tokenFromUrl);
  }, []);
  ...
  function connectOutlook() {
    ...
    if (!clientId) return alert("Missing VITE_MICROSOFT_CLIENT_ID. Add your Azure app client ID to Vercel, then redeploy.");
    const params = new URLSearchParams({
      client_id: clientId,
      response_type: "token",
      redirect_uri: getOutlookRedirectUri(),
      response_mode: "fragment",
      scope: "https://graph.microsoft.com/User.Read https://graph.microsoft.com/Calendars.Read",
  ...
  return (
    <section className="page">
      ...
      <Panel title="Microsoft Outlook connection" chip={graphToken ? "Connected" : "Setup required"}>
        <div className="outlookActions" style={{ marginBottom: 10 }}>
          <button className="primary" type="button" onClick={graphToken ? fetchOutlookAppointments : connectOutlook} disabled={graphLoading}>
            {graphLoading ? "Loading Outlook..." : graphToken ? "Load Outlook appointments" : "Connect Outlook"}
          </button>
          {graphToken && <button type="button" onClick={disconnectOutlook}>Disconnect</button>}
        ...
        <p className="outlookHelp">Requires Microsoft Azure app registration with this site URL as the redirect URI. Calendar access is read-only.</p>
        {graphError && <p className="bad">{graphError}</p>}
      </Panel>
      ...
      <div className="outlookGrid" style={{ marginTop: 14 }}>
        ...
        <Panel title="Outlook import queue" chip="Review before scheduling">
          ...
          <div className="outlookList">
            ...
            {!visibleAppointments.length && <p className="muted">No Outlook appointments ready. Connect Outlook and load appointments, or add one manually.</p>}
          </div>
        </Panel>
      </div>
...
function readOutlookTokenFromUrl() {
  if (typeof window === "undefined") return "";
