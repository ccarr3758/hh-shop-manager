...
function MobileManager({ jobs, ctx, reload, setEditingJob, selectedDate }) {
  ...
  async function completeJob(job) {
    ...
    if (error) return alert(error.message);
    notifyUser(`Completed: ${job.vehicle || "job"}`);
    await reload();
  }
...
function OutlookCalendar({ jobs, ctx, reload, selectedDate, setSelectedDate }) {
  ...
  function addManualAppointment(e) {
    ...
    const nextAppointment = makeOutlookAppointment({
      title,
      date: manual.date || selectedDate || todayIso(),
      start: manual.start || "08:00",
      body: manual.body || "",
    });
    ...
    <section className="page">
      <div className="hero">
        <div>
          ...
          <h3>Grab appointments, review, then push them to the shop schedule</h3>
          <p>This keeps Outlook from auto-filling bad jobs. Nothing becomes a shop job until you review it and click Import as Job.</p>
        </div>
        <div className="heroMetric">
          <span>Queued Appointments</span>
          <strong>{appointments.length}</strong>
        </div>
      </div>

      <div className="outlookGrid">
        <Panel title="Add Outlook appointment" chip="Manual / paste">
          <form className="outlookForm" onSubmit={addManualAppointment}>
            <label>
              Appointment Title
              <input value={manual.title} onChange={(e) => setManual({ ...manual, title: e.target.value })} placeholder="Example: Smith - 2023 F-250 Airlift" />
            </label>
            <label>
              Date
            ...
            <label>
              Notes / Body
              <textarea value={manual.body} onChange={(e) => setManual({ ...manual, body: e.target.value })} placeholder="Paste Outlook appointment body, phone, vehicle, requested work, etc." />
            </label>
          ...
          <div className="outlookForm" style={{ marginTop: 14 }}>
            <label>
              Paste Outlook appointments
              <textarea value={pasteText} onChange={(e) => setPasteText(e.target.value)} placeholder={"One per line:\nSmith - 2023 F-250 Airlift | 2026-06-18 | 09:00 | WirelessOne notes"} />
            </label>
            <button type="button" onClick={importPastedAppointments}>Parse pasted appointments</button>
            <p className="outlookHelp">Recommended format: Title | Date | Start Time | Notes. The app also tries to detect dates and times from plain copied text.</p>
          </div>
        </Panel>
        ...
        <Panel title="Outlook import queue" chip="Review before scheduling">
          ...
          <div className="outlookList">
              ...
              return (
                <article className={`outlookCard ${imported ? "imported" : ""}`} key={appt.id}>
                  <div className="outlookCardTop">
                    <div>
                      <h4>{appt.title}</h4>
                      <p>{formatOutlookDate(appt.date)} • {formatTime(appt.start)} </p>
                    </div>
                    <span className={`outlookBadge ${imported ? "imported" : ""}`}>{imported ? "Imported" : "Ready"}</span>
                  </div>
                  {appt.body && <p>{appt.body}</p>}
                  <div className="outlookActions">
                    <button type="button" onClick={() => { setSelectedDate(appt.date || todayIso()); setSelectedAppointment(appt); }} disabled={imported}>
                      Import as Job
                    </button>
            ...
            {!appointments.length && <p className="muted">No Outlook appointments queued. Add one manually or paste copied Outlook appointments.</p>}
          </div>
        </Panel>
      </div>
